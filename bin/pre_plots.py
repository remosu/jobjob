...
#ELBData = nametuple('ELBData', 'gretptext prefix')    
#electdata = ELBData("grep 'oid %(part)s ele' %(log_file)s | awk '{print $9}'  > %(out)s/ElForce_c%(part)s.dat", 
#                    "ElForce_c")
#hydrodata = ELBData("grep 'to colloid %(part)s' %(log_file)s | awk '{print $9}'  > %(out)s/HydroForce_c%(part)s.dat", 
#                    "HydroForce_c")

#def grep_data(elbdata, directory):
#    out_dir = os.path.join(directory, 'out')
#    log_files = glob.glob(os.path.join(out_dir, 'log_par_*'))
#    log_file = log_files[0] if log_files else None
#    force_datas = []
#    for part in (10, 11):
#        comm = elbdata.greptext % {'part':part, 'log_file':log_file, 'out':out_dir}
#        os.system(comm)

#def plot_data(elbdata, directory):
#    pass    
...
for directory in sys.argv[1:]:
    ...
    if not os.path.exists(ef10_filename) or not os.path.exists(ef11_filename):
        for comm in  ["grep 'oid %(part)s ele' %(log_file)s | awk '{print $9}'  > %(out)s/ElForce_c%(part)s.dat" 
               % {'part':particle, 'log_file':log_file, 'out':out_dir} for particle in ('10', '11')]:
            print '>>>: ', comm
            os.system(comm)
    ...
    if not os.path.exists(hf10_filename) or not os.path.exists(hf11_filename):
        for comm in ["grep 'to colloid %(part)s' %(log_file)s | awk '{print $9}'  > %(out)s/HydroForce_c%(part)s.dat"
                     % {'part':particle, 'log_file':log_file, 'out':out_dir} for particle in ('10', '11')]:
            print '>>>: ', comm
            os.system(comm)
